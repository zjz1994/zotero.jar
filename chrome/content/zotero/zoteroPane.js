...
var ZoteroPane = new function ()
{
    ...
    */
	...
	this.deleteSelectedItems = function (force, fromMenu) {
		...
		if (collectionTreeRow.isSearch() || collectionTreeRow.isUnfiled() || collectionTreeRow.isDuplicates()) {
			if (!force) {
				return;
			}
			var prompt = toTrash;
    ...
    */
	...
	this.buildCollectionContextMenu = async function () {
		...
		if (collectionTreeRow.isDuplicates() || collectionTreeRow.isUnfiled()) {
	...
	this.newNote = Zotero.Promise.coroutine(function* (popup, parentKey, text, citeURI) {
		...
		if (this.collectionsView.selectedTreeRow.isCollection()) {
