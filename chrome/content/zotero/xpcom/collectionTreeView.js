		...
		for (let i = 0; i < ids.length; i++) {
			...
			switch (type) {
				...
				case 'group':
					...
					row < this.rowCount && this.getLevel(row) > level);
   ...
   if (treeRow.ref.updating) {
   	collectionType += '-updating';
		...
		case 'header':
			...
			if (treeRow.ref.id == 'feed-libraries-header') {
				collectionType = 'feedLibrary';
			} else
			if (treeRow.ref.id == 'commons-header') {
...
Zotero.CollectionTreeView.prototype.getCellProperties = function (row, col, prop) {
	...
	if (treeRow.ref && treeRow.ref.unreadCount) {
		props.push('unread');
	}
