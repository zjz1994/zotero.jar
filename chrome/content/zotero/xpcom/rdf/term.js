...
$rdf.Literal = function (value, lang, datatype) {
  this.value = value;
  if (lang == "" || lang == null) this.lang = undefined;else
  this.lang = lang; // string
  if (datatype == null) this.datatype = undefined;else
...
$rdf.term = function (val) {
  ...
  if (typeof val == 'number') {
    var dt;
    if (('' + val).indexOf('e') >= 0) dt = $rdf.Symbol.prototype.XSDfloat;else
    if (('' + val).indexOf('.') >= 0) dt = $rdf.Symbol.prototype.XSDdecimal;else
...
$rdf.Formula.prototype.fromNT = function (str) {
  ...
  if (ch == '"') {
    ...
    if (k < len - 1) {
      if (str[k + 1] == '@') lang = str.slice(k + 2, len);else
      if (str.slice(k + 1, k + 3) == '^^') dt = $rdf.fromNT(str.slice(k + 3, len));else
